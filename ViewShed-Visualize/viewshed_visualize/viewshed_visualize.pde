...
double calcSlope(Cell target) {
    double deltaZ = target.getElevation() - observer.getElevation() + (observer.getElevation() < 0 ? HEIGHT_TOLERENCE : -HEIGHT_TOLERENCE);
    double deltaXY = Math.sqrt(Math.pow(target.getX() - observer.getX(), 2) + Math.pow(target.getY() - observer.getY(), 2));
...
void setup() {
  ...
  for(int i = HEADERS_SIZE; i < lines.length; i++) {
    ...
    for(int col = 0; col < cols; col++) {
      grid[col][row] = new Cell(col, row, 2, 2, Integer.parseInt(items[col]));
    }
  }
  ...
  for (int i = 0; i < slopes.length; i++) {
    for (int j = 0; j < slopes[0].length; j++) {
        Cell target = new Cell(x1 + j, y1 + i, 2, 2, grid[x1 + j][y1 + i].getElevation());
  ...
  for(Cell cell : bresenhamCircle) {
   ...
   if(!ANIMATE) {
      ...
      for(Cell lineCell : bresenhamLine) {
        double slope = slopes[lineCell.getY() - y1][lineCell.getX() - x1];
        if(slope >= maxSlope) {
          maxSlope = slope;
